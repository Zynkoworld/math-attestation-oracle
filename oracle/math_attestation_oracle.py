#!/usr/bin/env python3
"""math-attestation-oracle -- a deterministic attestation-integrity decider for the `math` knowledge domain.

An **oracle** *deterministically decides* the truth of a case. This one decides, for a `math` knowledge
capsule, whether it is a well-formed, provenance-carrying attestation. VALID iff: domain=='math', a non-empty
...
def decide(c):
    if c.get("domain") != "math": return "INVALID"
...
def _run_probes(path):
    ...
    print("math attestation oracle: probes=%d | recall=%.3f | false_positives=%d | verdict=%s"%(len(probes),recall,fp,"PASS" if ok else "FAIL"))
